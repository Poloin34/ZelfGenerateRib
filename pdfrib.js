async function GenerateDoc() {
  ...
  const nom_prenom = "Elliot Goykhman"//form.getTextField('nom_prenom')
  const addresse_rue = "Audēju iela 15 - 4, Centra rajons"//form.getTextField('adresse_rue')
  const addresse_cpville = "Rīga, LV-1050, Lettonie"//form.getTextField('adresse_cp-ville')
  ...
  const date = "01/01/1970"//form.getTextField('date_of_the_day')

  nom_prenom.setText(nom_prenom);//document.getElementById("name").value)
  addresse_rue.setText(adresse_rue);//document.getElementById("addressStreet").value)
  addresse_cpville.setText(adresse_cpville);//document.getElementById("addressZipcode").value + " " + document.getElementById("addressCity").value)
